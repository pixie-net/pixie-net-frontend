...
int MainFrame::ShowTraces() {
    ...
    if (chkAdc0->IsOn()) {
        if (TMath::MaxElement(NTRACE_SAMPLES, g0->GetY()) > traceMax)
            traceMax = TMath::MaxElement(NTRACE_SAMPLES, g0->GetY());
        if (TMath::MinElement(NTRACE_SAMPLES, g0->GetY()) < traceMin)
            traceMin = TMath::MinElement(NTRACE_SAMPLES, g0->GetY());
    }
    
    if (chkAdc1->IsOn()) {
        if (TMath::MaxElement(NTRACE_SAMPLES, g1->GetY()) > traceMax)
            traceMax = TMath::MaxElement(NTRACE_SAMPLES, g1->GetY());
        if (TMath::MinElement(NTRACE_SAMPLES, g1->GetY()) < traceMin)
            traceMin = TMath::MinElement(NTRACE_SAMPLES, g1->GetY());
    }
    
    if (chkAdc2->IsOn()) {
        if (TMath::MaxElement(NTRACE_SAMPLES, g2->GetY()) > traceMax)
            traceMax = TMath::MaxElement(NTRACE_SAMPLES, g2->GetY());
        if (TMath::MinElement(NTRACE_SAMPLES, g2->GetY()) < traceMin)
            traceMin = TMath::MinElement(NTRACE_SAMPLES, g2->GetY());
    }
    
    if (chkAdc3->IsOn()) {
        if (TMath::MaxElement(NTRACE_SAMPLES, g3->GetY()) > traceMax)
            traceMax = TMath::MaxElement(NTRACE_SAMPLES, g3->GetY());
        if (TMath::MinElement(NTRACE_SAMPLES, g3->GetY()) < traceMin)
            traceMin = TMath::MinElement(NTRACE_SAMPLES, g3->GetY());
    }
